...
export function defineReactive (
  ...
  /**
   * 对于基本类型属性，则只有一个闭包dep实例作为依赖收集器，
   * 而对于引用类型，则为了外不能访问到，则又添加了一个__ob__.dep（childOb.dep）
   * 依赖收集器，主要用于（set、del）方法的调用。
   * 
   * 
   */
  /**
   * 对引用类型来说，修改其中属性并不会引起该引用属性set的触发，
   * 因为引用对象的指向没有发生变化，也就不会触发更新，
   * 所以提供一个该引用的__ob__，在增删时调用。
   * 所以这时当触发该引用属性的get时，除了本身的闭包dep要收集一份依赖
   * 之外，还要让__ob__.dep也收集一份，同时如果该属性为数组时，还得遍历
   * 数组成员，如果数组成员是引用属性的话，都要让各成员的__ob__.dep都收集
   * 一份
   * 
   * 可能会有疑惑，子属性不也有各自的get方法收集么，实际上，这里对父引用
   * 对象的访问，甚至增删，不会间接触发子属性的get。只会触发父引用的get，
   * 
   */
